...
#
# Simulates the feather frame data interface to the NanoLabs by
# connecting to multiple serial devices provided by the user and then
# sending the serial packets contained in the nff-packets.txt file
...
def main():
    ...
    userInput = input('Enter the ports to connect to: ')
    print('')
    ...
    for lines in all_lines:
        ...
        if (keyboard.is_pressed("left")) and (speed != 1):
            speed = speed / 2
        # Right arrow code speeds up (max is x16).
        elif (keyboard.is_pressed("right")) and (speed != 16):
            speed = speed * 2
        ...
        print('NFF simulation running' + '.' * round((packet_counter % 12) / 3))
        print('')
        ...
        # Display how far along the simulation is with a neat loading bar
        # (the magic 20 number is to prevent the simulation from obnoxiously
        # ending while displaying 99%, I'm a little OCD).
        print('Simulation progress: [' +
              '#' * round(packet_counter / (num_lines / PROGRESS_BAR_LENGTH)) +
              ' ' * round(PROGRESS_BAR_LENGTH - (packet_counter / (num_lines / PROGRESS_BAR_LENGTH))) +
        ...
        # Delay is chosen to result in a send rate of roughly 10Hz,
        # call isn't very precise at ms level.
        time.sleep(0.07)
