...
Sample python code for NanoLab payload on the NanoRacks Feather Frame. 
Program opens up a serial connection and listens to decode any incoming 
feather frame packets. Default values and ports are especially for use on a 
raspberry pi zero through the UART0 interface. All code is provided as-is by 
NanoRacks to help customers develop flight code for their Feather Frame payloads
as quickly and easily as possible.

Note:
- Use pip to download the pyserial library (pip install pyserial) 
    if it's missing.
- For pi zero, a line must be added to /etc/rc.local to run the python 
    script on startup. For example: python /home/pi/nff-sample.py &. 
    The ending & runs the program in the background.
"""
...
# This is specific to the pi zero. Must have serial enabled and console over 
# serial disabled.
...
def main():
    ...
    while True:
        # Check for any available data in the input serial buffer according to
        # the polling rate.
        ...
        # If the experiment hasn't run yet then check against the start 
        # conditions to decide if it should be started. Example start condition 
        # is altitude > 20000 ft.
        if not EXPERIMENT_RUN:
            if FLIGHT_DATA['altitude'] > 20000:
                start_experiment()
                EXPERIMENT_RUN = True
        # If the experiment has been run but has not completed yet, then check
        # against the stop condition to decide if it should be finished. Example
        # stop condition is altitude > 50000 ft.
